...
def get_computation_for_map_reduce_form(
  ...
  next_parameter_type = computation_types.StructType([
      (mrf.server_state_label,
       computation_types.FederatedType(mrf.update.type_signature.parameter[0],
                                       placements.SERVER)),
...
def get_map_reduce_form_for_iterative_process(
    ...
    *,
    tff_internal_preprocessing: Optional[BuildingBlockFn] = None,
) -> Tuple[computation_base.Computation, forms.MapReduceForm]:
  """Constructs `tff.backends.mapreduce.MapReduceForm` given iterative process.

  Args:
    ip: An instance of `tff.templates.IterativeProcess` that has a `next`
      computation that is compatible with MapReduce form. Iterative processes
      are only compatible if `initialize_fn` returns a single federated value
      placed at `SERVER` and `next` takes exactly two arguments. The first must
      be the state value placed at `SERVER` and must be assignable from the
      return type of `initialize`. The `next` computation also must return
      exactly two values, and the type of the first element of the result must
      be assignable to the first element of the `next` computation's parameter.
    grappler_config: An optional instance of `tf.compat.v1.ConfigProto` to
      configure Grappler graph optimization of the TensorFlow graphs backing the
      resulting `tff.backends.mapreduce.MapReduceForm`. These options are
      combined with a set of defaults that aggressively configure Grappler. If
  ...
  Returns:
    A tuple of the following:
      - A validated and simplified `initialize` computation from the provided
        `tff.template.IterativeProcess`.
      - An instance of `tff.backends.mapreduce.MapReduceForm` equivalent to the
      `next` computation of the provided `tff.templates.IterativeProcess`.
  ...
  # Generate the MapReduceForm from the `next` computation.
  mrf = get_map_reduce_form_for_computation(
      ip.next,
      grappler_config,
      tff_internal_preprocessing=tff_internal_preprocessing)

  # Validate and simplify the `initialize` computation.
  initialize = get_state_initialization_computation_for_map_reduce_form(
      ip.initialize, grappler_config=grappler_config)

  return initialize, mrf
